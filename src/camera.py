...
class DeviceMetadata:
    Name = "FLI Camera"
    Version = "1.0.0"
    Description = "FLI Camera ASCOM Alpaca Driver via libfli"
    DeviceType = "Camera"
...
def _connected_property(device: CameraDevice, getter, params):
    ...
    `getter` is a callable so the underlying property is read only after
    the connection check passes — important because many getters call
    into a hardware library that may not be loaded if no device is
    attached.
    """
    ...
    try:
        ...
        return PropertyResponse.create(
            value=None,
            client_transaction_id=params.client_transaction_id,
            error=DriverException(0x500, "Camera property read failed", ex),
        ).model_dump()
    return PropertyResponse.create(
        value=value,
        client_transaction_id=params.client_transaction_id,
...
async def cooleron_put(devnum: int, CoolerOn: Annotated[str, Form()], params: AlpacaPutParams = Depends(alpaca_put_params)):
    device = get_device(devnum)
    # Validate CoolerOn *before* the connected check so bad values yield 400.
    cooler_on = to_bool(CoolerOn)
    if not device.connected:
        return MethodResponse.create(
            client_transaction_id=params.client_transaction_id,
            error=NotConnectedException(),
...
async def electronsperadu(devnum: int, params: AlpacaGetParams = Depends()):
    # libfli exposes no gain / e-/ADU API for FLI CCDs.
    get_device(devnum)
    return PropertyResponse.create(
        value=None,
        client_transaction_id=params.client_transaction_id,
...
# FLI CCD cameras have no gain control in libfli — Gain/GainMax/GainMin/Gains
# all report NotImplemented per the ICameraV4 contract.
...
# FLI CCD cameras have no offset control in libfli.
...
async def offsetmax(devnum: int, params: AlpacaGetParams = Depends()):
    ...
    return PropertyResponse.create(
        value=None,
        client_transaction_id=params.client_transaction_id,
        error=NotImplementedException("OffsetMax"),
    ).model_dump()
...
async def offsetmin(devnum: int, params: AlpacaGetParams = Depends()):
    ...
    return PropertyResponse.create(
        value=None,
        client_transaction_id=params.client_transaction_id,
        error=NotImplementedException("OffsetMin"),
    ).model_dump()
